...
import random
import numpy as np
from occupancy_grid_map.rrt.rrt_utils.tree import Tree
from occupancy_grid_map.rrt.rrt_utils.geometry import steer
...
class RRTBase(object):
    ...
    def connect_to_point(self, tree, x_a, x_b):
        ...
        """
        if self.trees[tree].V.count(x_b) == 0 and self.X.collision_free(x_a, x_b):
    ...
    def can_connect_to_goal(self, tree):
        ...
        if self.x_goal in self.trees[tree].E and x_nearest in self.trees[tree].E[self.x_goal]:
            ...
            return True
        if self.X.collision_free(x_nearest, self.x_goal):  # check if obstacle-free
            return True
        return False
    ...
    def bound_point(self, point):
        ...
        return self.X.project_to_bound(point)
