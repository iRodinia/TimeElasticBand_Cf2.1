...
class trajGenerator:
    def __init__(self,waypoints,max_vel = 5,gamma = 100):
        ...
        self.gamma = gamma
        self.order = 10 # Must be 10, make matrixes square
    ...
    def get_constraints(self,T):
        ...
        A = np.zeros((10*n, self.order*n))
        B = np.zeros((10*n, self.dim))
    ...
    def get_coef_mat(self):
        seg_num = int(self.coeffs.shape[0] / self.order)
        coef_mat = self.coeffs.T
        mat = np.zeros((seg_num, 3*self.order+1))
        durations = np.diff(self.TS)
        mat[:,0] = durations

        for seg in range(seg_num):
            mat[seg,1:self.order+1] = coef_mat[0,seg*self.order:(seg+1)*self.order]
            mat[seg,self.order+1:2*self.order+1] = coef_mat[1,seg*self.order:(seg+1)*self.order]
            mat[seg,2*self.order+1:3*self.order+1] = coef_mat[2,seg*self.order:(seg+1)*self.order]
